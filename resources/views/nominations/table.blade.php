<div class="table-responsive">
    <table class="table" id="nominations-table">
        ...
        @foreach($nominations as $nomination)
            <tr>
                <td><a href="{{ route('nominations.show', [$nomination->id]) }}">{{ $nomination->name }}</a> ({{ $nomination->gender }})</td>
            <td>{{ $nomination->business_name }}</td>
            <td>{{ $nomination->no_of_nominations }}</td>
            <td>
                @if($nomination->is_admin_selected == 1) yes
            @else no
            @endif

            </td>
            <td>@if($nomination->has_won == 1) yes
                @else no
                @endif</td>
                <td>
